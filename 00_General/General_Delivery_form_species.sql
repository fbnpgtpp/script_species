...
where ps.projectid = 98
	...
	replace(concat('Trees preregistred (',coalesce(tren.translationlabel, sp.nom_latin),
    ') : %DOL{',concat(species_code,'_nb_trees_preregistred_pull}')),'%DOL','$') as labelen,
	replace(concat('Pohon pra-terdaftar (',coalesce(tr.translationlabel, sp.nom_latin),
    ') : %DOL{',concat(species_code,'_nb_trees_preregistred_pull}')),'%DOL','$') as labelin,
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 98
	...
	concat(coalesce(tren.translationlabel, sp.nom_latin),' - Number of delivered trees') as labelen,
	concat(coalesce(tr.translationlabel, sp.nom_latin),' - Jumlah pohon yang dikirim') as labelin,
	'' as hinten,
	'' as hintin,
	'TRUE' as required,
	'' as apearence,
