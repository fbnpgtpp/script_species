...
left join kobo_import.translations tr on tr.id = sp.translationid and  tr.languecode = 'ch'
left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
where ps.projectid = 103
	union all
	--ligne 3 (OK)
	...
	concat(coalesce(tr.translationlabel, sp.nom_latin) , ' 数量') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'TRUE' as required,
	'' as apearence,
	...
	(SELECT replace(concat('.>= 0 and (%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join kobo_import.translations tr on tr.id = sp.translationid and  tr.languecode = 'ch'
		left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
		where ps.projectid = 103) as constrainte,
...
where ps.projectid = 103
	...
	replace(concat('<span style="color:#099665">', coalesce(tr.translationlabel, sp.nom_latin) , ' 占比 :</span> %DOL{',species_code,'_share} %'),'%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 103
	...
	(SELECT replace(concat('%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join kobo_import.translations tr on tr.id = sp.translationid and  tr.languecode = 'ch'
		left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
		where ps.projectid = 103) as calculation
	from kobo_import.species sp
inner join kobo_import.projectspecies ps on ps.speciesid = sp.id
left join kobo_import.translations tr on tr.id = sp.translationid and  tr.languecode = 'ch'
left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
where ps.projectid = 103
	...
	replace(concat(	'<span style="color:#E6361D">剩余可分配数量 :</span> %DOL{trees_available_after_', species_code, '} 棵'), '%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 103
	...
	replace('**剩余可分配数量 :** %DOL{remaining_trees} 棵','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
	...
	replace('<span style="color:red">错误: **还有 %DOL{remaining_trees} 棵树未分配。** 请调整树种分配或3.3.中的种植数量.</span>','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
	...
	replace('**地块预登记种植总数 :** : %DOL{parcel_total_nb_of_trees}','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
