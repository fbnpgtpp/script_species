...
with cte as(
	...
	coalesce(tr.translationlabel, sp.nom_latin) as labelen,
	'' as labelin,
	'' as hinten,
	'' as hintin,
	'' as required,
...
left join kobo_import.translations tr on tr.id = sp.translationid and tr.languecode = 'gh'
left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
where ps.projectid = 96
	union all
	--ligne 3 (OK)
	select sp.id as speciesid, 3 as  roworder, 
	'integer' as type_,
	concat(species_code,'_nb_of_trees') as name_,
	concat('**',coalesce(tr.translationlabel, sp.nom_latin) , ' number of trees :**') as labelen,
	...
	(SELECT replace(concat('.>= 0 and (%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join kobo_import.translations tr on tr.id = sp.translationid and tr.languecode = 'gh'
		left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
		where ps.projectid = 96) as constrainte,
...
where ps.projectid = 96
	...
	replace(concat('<span style="color:#099665">', coalesce(tr.translationlabel, sp.nom_latin) , ' share :</span> %DOL{',species_code,'_share} %'),'%DOL','$') as labelen,
	'' as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
...
where ps.projectid = 96
	...
	(SELECT replace(concat('%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join kobo_import.translations tr on tr.id = sp.translationid and tr.languecode = 'gh'
		left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
		where ps.projectid = 96) as calculation
	from kobo_import.species sp
inner join kobo_import.projectspecies ps on ps.speciesid = sp.id
left join kobo_import.translations tr on tr.id = sp.translationid and tr.languecode = 'gh'
left join kobo_import.translations tren on tren.id = sp.translationid and tren.languecode = 'en'
...
where ps.projectid = 96
	...
	concat(coalesce(tr.translationlabel, sp.nom_latin) , ' Total number of trees : <span style="color:red">${',species_code,'_total_nb} trees</span>') as labelen,
	'' as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
