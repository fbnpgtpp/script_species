...
with cte as(
	...
	'Selección de especies' as labelin,
	'' as hinten,
	'' as hintin,
	'false' as required,
...
left join schemav5.translations tr on tr.id = sp.translationid and tr.languagecode = 'mx'
left join schemav5.translations tren on tren.id = sp.translationid and tren.languagecode = 'en'
where ps.projectid = 104
	union all
	--ligne 3 (OK)
	...
	concat(coalesce(tr.translationlabel, sp.latinname) , ' Número de árboles') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'TRUE' as required,
	'' as apearence,
	...
	(SELECT replace(concat('.>= 0 and (%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join schemav5.translations tr on tr.id = sp.translationid and tr.languagecode = 'mx'
		left join schemav5.translations tren on tren.id = sp.translationid and tren.languagecode = 'en'
		where ps.projectid = 104) as constrainte,
	...
	'Ingresó un número negativo o un número mayor que el número de árboles restantes. Por favor corríjalo.' as constraintmessother, --CHANGE OR DELETE TRANSLATION
	'0' as default,
	'' as calculation
	from schemav5.species sp
inner join schemav5.projectspecies ps on ps.speciesid = sp.id
...
where ps.projectid = 104
	...
	replace(concat('<span style="color:#099665">Proporción ', coalesce(tr.translationlabel, sp.latinname) , ' :</span> %DOL{',code,'_share} %'),'%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 104
	...
	(SELECT replace(concat('%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		...
		left join schemav5.translations tr on tr.id = sp.translationid and tr.languagecode = 'mx'
		left join schemav5.translations tren on tren.id = sp.translationid and tren.languagecode = 'en'
		where ps.projectid = 104) as calculation
	from schemav5.species sp
inner join schemav5.projectspecies ps on ps.speciesid = sp.id
left join schemav5.translations tr on tr.id = sp.translationid and tr.languagecode = 'mx'
left join schemav5.translations tren on tren.id = sp.translationid and tren.languagecode = 'en'
where ps.projectid = 104
	...
	replace(concat(	'<span style="color:#E6361D">Número de árboles restantes :</span> %DOL{trees_available_after_', code, '} árboles'), '%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 104
	...
	'Advertencia: ¡Has distribuido menos árboles de lo que había planeado! **Todavía quedan ${remaining_trees} árboles restantes. ** Por favor, distribuya todos los árboles o modifique el "número de árboles a sembrar" en la sección anterior.' as constraintmessother, --CHANGE OR DELETE TRANSLATION
	'' as default,
	(SELECT replace(concat('%DOL{nb_of_trees_before_dispatching} - (',string_agg(concat('coalesce(${',
		concat(code,'_nb_of_trees'),
		'},0)'), ' + '),')'),'%DOL','$')
	...
	replace('**Número de árboles restantes :** Proporción %DOL{remaining_trees}','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
	...
	replace('<span style="color:red">Advertencia: ¡Has distribuido menos árboles de lo que había planeado! **Todavía quedan %DOL{remaining_trees} árboles restantes. ** Por favor, distribuya todos los árboles o modifique el "número de árboles a sembrar" en la sección anterior.</span>','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
	...
	'Recapitulación de la parcela' as labelin,
	'' as hinten,
	'' as hintin,
	'' as required,
	'field-list' as apearence,
	...
	replace('**Número total de árboles :** : %DOL{parcel_total_nb_of_trees}','%DOL','$') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
with cte as(
	...
	'**Número total de árboles por especie :**' as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
...
where ps.projectid = 104
	...
	concat(coalesce(tr.translationlabel, sp.latinname) , ' Número total de árboles : <span style="color:red">${',code,'_total_nb} árboles</span>') as labelin, --CHANGE OR DELETE TRANSLATION
	'' as hinten,
	'' as hintin,
	'' as required,
	'' as apearence,
